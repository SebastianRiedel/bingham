...
#include <vector>       // for tree-buckets
//----------------------------------------------------------------------
// Limits
...
DLL_API ANNpoint annAllocPt(
  int       dim,    // dimension
  ANNcoord    c);   // coordinate value (all equal)
...
DLL_API inline ANNpoint annAllocPt ( int dim )    // allocate 1 point
{
  ANNpoint p = new ANNcoord[dim];
  return p;
}

DLL_API inline ANNpoint annAllocPt ( int dim, ANNcoord c )    // allocate 1 initialized point
{
  ANNpoint p = new ANNcoord[dim];
...
DLL_API inline ANNpointArray annAllocPts ( int n, int dim )   // allocate n pts in dim
{
...
DLL_API inline ANNpointArray annAllocPts ( int n, int dim, ANNcoord c )   // allocate n initialized pts in dim
{
  ANNpointArray pa = new ANNpoint[n];     // allocate points
  ANNpoint      p  = annAllocPt ( n*dim, c );
  for ( int i = 0; i < n; i++ )
  {
    pa[i] = & ( p[i*dim] );
  }
...
DLL_API inline void annDeallocPt ( ANNpoint &p )          // deallocate 1 point
{
  //if (p == NULL)
  //  std::cerr << "[annDeallocPt] Trying to deallocate a NULL point." << std::endl;
  delete [] p;
  p = NULL;
}

DLL_API inline void annDeallocPts ( ANNpointArray &pa )     // deallocate points
{
  if (pa == NULL)
    std::cerr << "[annDeallocPts] Trying to deallocate a NULL point array." << std::endl;
  delete [] pa[0];              // dealloc coordinate storage
  delete [] pa;               // dealloc points
  pa = NULL;
}

DLL_API inline ANNpoint annCopyPt ( int dim, ANNpoint source )  // copy point
{
  ANNpoint p = new ANNcoord[dim];
...
//    n_max         Maximum number of points that are allowed in the tree
...
class DLL_API ANNkd_tree: public ANNpointSet
{
  protected:
    ...
    /// Vector containing a reference towards the tree's buckets
    std::vector<ANNkdBucket>* kd_leaves;
  ...
  public:
    ...
    /// Return the reference towards the tree's buckets
    inline std::vector<ANNkdBucket>* annGetKdLeaves ()
    {
      return kd_leaves;
    }
};
