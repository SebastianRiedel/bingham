...
// Init lazy randperm (randperm on demand)
void init_lazy_randperm(int n, int *step, int *i0)
{
  init_rand();
  ...
  *step = big_primes[rand() % 100];
}

int lazy_randperm_request(int n, int step, int *i0) {
  int idx = *i0;
  *i0 = (*i0 + step) % n;
  return idx;
}
