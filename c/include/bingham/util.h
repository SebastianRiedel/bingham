...
typedef struct {
  ...
  vertex_t *vertices;
  edge_t *edges;
  //int _edge_capacity;  // internal--do not modify
} graph_t;

//graph_t *graph_new(int num_vertices, int edge_capacity);    // create a new graph
void graph_free(graph_t *g);                                // free a graph
//void graph_add_edge(graph_t *g, int i, int j);              // add an edge to a graph
